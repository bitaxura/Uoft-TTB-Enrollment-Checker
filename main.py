...
sections = ['YOUR_SECTIONS_HERE'] # List of sections to check, e.g., ['PRA0101', 'PRA0119']
...
for course in course_code:
    payload = {
        "courseCodeAndTitleProps": {"courseCode": course, "courseTitle": "", "courseSectionCode": ""},
        "departmentProps": [],
        "campuses": [],
        "sessions": sessions,
        "requirementProps": [],
    ...
    for section in sections:
        ...
        if current_enrollement < max_enrollement:
            message = f"There are {max_enrollement - current_enrollement} spots available in section {section} in course {course}."
            send_discord_message(message)
        else:
            message = f"Section {section} of course {course} is full."
            send_discord_message(message)
